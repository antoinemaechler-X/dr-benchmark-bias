"""
Bootstrap the remaining definable benchmarks on Modal, DETACHED so the laptop
can be closed. Results are written to the same Modal Volume as v2 and downloaded
afterwards.

Benchmarks (every benchmark where the semi-synthetic approach is well-defined and
not already run; the 6 fully-observed benchmarks are excluded because there is no
real missingness to learn a masking model from):
    ai2d_test, mmbench_v11  -- real missingness (~2-3%), 250+ models each
    agentdojo, bfcl, mtbench -- trivial missingness (<1%), included for completeness

Launch (returns immediately; safe to close laptop):
    python3.10 -m modal run --detach bootstrap_modal_v3.py --iters 1000

Download results when ready:
    modal volume get dr-bootstrap-vol bootstrap_results_v3.csv  ./
    modal volume get dr-bootstrap-vol bootstrap_permodel_v3.npz ./

Check progress any time:
    modal app list          # shows dr-bootstrap-v3 running
"""

import modal

app = modal.App("dr-bootstrap-v3")
...
# per-benchmark seed chunk size (small for the big matrices so each chunk stays
# well under the function timeout; large for the tiny ones to cut overhead)
BENCH_CHUNK = {
    "ai2d_test": 5, "mmbench_v11": 5, "bfcl": 5,
    "agentdojo": 25, "mtbench": 25,
}
...
@app.function(image=image, timeout=21600, volumes={"/out": vol})
def orchestrate(iters, benchmarks):
    ...
    df.to_csv("/out/bootstrap_results_v3.csv", index=False)
    ...
    np.savez_compressed("/out/bootstrap_permodel_v3.npz", **npz)
    vol.commit()
    print(f"Wrote bootstrap_results_v3.csv ({len(df)} rows) and "
          f"bootstrap_permodel_v3.npz ({len(permodel)} keys) to the Volume.")
    return len(df)
...
def main(iters: int = 1000):
    ...
    print("  modal volume get dr-bootstrap-vol bootstrap_results_v3.csv  ./")
    print("  modal volume get dr-bootstrap-vol bootstrap_permodel_v3.npz ./")
